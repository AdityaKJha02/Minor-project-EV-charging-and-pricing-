function PED = PED_DPM_from_BSA_Excel(params, BSA_excel, PED_excel)

fprintf('\n===== ENTERING PED_DPM_from_BSA_Excel =====\n');
...
timeGrid_min  = params.timeGrid_min;
...
fprintf('rho_PUR length = %d\n', length(rho_PUR));
fprintf('dt = %.4f\n', dt);
fprintf('Number of fixed costs = %d\n', length(C_FIXED_TOTAL));
...
for i = 1:nFCS
    ...
    % Extract 144 time slots (rows 2–145)
    P_EV = M(:,3);
    P_BC = M(:,5);
    P_BD = M(:,6);
    ...
    % Prevent negative grid purchase
    E_GRID_h(E_GRID_h < 0) = 0;

    fprintf('Total EV energy = %.2f kWh\n', sum(E_EV_h));
    fprintf('Total Grid energy = %.2f kWh\n', sum(E_GRID_h));
...
while basePrice <= baseMax
    ...
    for i = 1:nFCS
        ...
        %% --------------------------------
        % Generate PED price profile
        %% --------------------------------
        [~, ~, ~, ~, ~, price_h, ~] = compute_PED_DPM_sigmoid( ...
            timeGrid_min, ...
            RTP_file, ...
        ...
        fprintf('Hourly price vector length = %d\n', length(price_h));

        %% convert 24 → 144 slots
        rhoSELL_vec = repelem(price_h,6);

        fprintf('Expanded price vector length = %d\n', length(rhoSELL_vec));
...
% OUTPUT
%% =========================================================
PED.basePrice     = basePrice;
PED.Total_Revenue = Total_Revenue;
PED.Total_Cost    = Total_Cost;
PED.PM            = PM;
...
if PED.PM >= params.PM_DES
    disp('✔ Desired profit margin achieved');
else
    disp('✘ Desired profit margin NOT achieved');
end
end

...
% FILES
%% =========================================================
EV_file     = 'EVFCS_10min_Arrival_Power_TOU_FINAL.xlsx';
PV_file     = 'PV_Results_6_NPV_10min.xlsx';
RTP_file    = 'RTP_grid_cost_10min.xlsx';

BSA_excel   = 'TEMP_BSA_TOU.xlsx';
...
SOC_init_vec = [50 55 50 55 50 50];   % one value per FCS
...
%% =========================================================
params.PM_DES       = 0.25;
params.basePrice0   = 9.55;  % 9.58 at 1100 obtained on 07/03
...
for k = 1:nB
    ...
    %% --------------------------------
    % RUN TOU OPTIMISATION
    %% --------------------------------
    TOU = TOU_from_BSA_Excel(params, BSA_excel);

    %% --------------------------------
    % STORE RESULTS
    %% --------------------------------
    BasePrice_vec(k) = TOU.basePrice;
    Revenue_vec(k)   = TOU.Total_Revenue;
    TotalCost_vec(k) = TOU.Total_Cost;
    PM_vec(k)        = TOU.PM;

    FixedCost_vec(k) = sum(C_FIX_vec) + sum(C_OandM_vec);
    VarCost_vec(k)   = TOU.Total_Cost - FixedCost_vec(k);
...
outputFile = 'TOU_BESS_Results.xlsx';
...
disp('Excel file generated: TOU_BESS_Results.xlsx');
...
title('TOU Optimized Base Price vs Battery Size');
...
title('TOU Profit Margin vs Battery Size');
...
fprintf('\nOptimal BESS = %d kWh\n', batterySizes(idx_min));
fprintf('Minimum price = %.2f Rs/kWh\n', Price_min);

%% =========================================================
% REGENERATE BSA FOR OPTIMAL BESS
%% =========================================================

...
for fcsID = 1:nFCS
    ...
    [N_frac, N_int, E_EV_DPM, tau_h, price_h, ~] = ...
    compute_PED_DPM_sigmoid( ...
        timeGrid_min, RTP_file, ...
        EV_file, sheetNames_EV, fcsID, ...
        base_price);

P_EV_DPM_kW = E_EV_DPM * 6;              % 10-min → kW
Revenue_DPM = price_h .* E_EV_DPM;
TotalRevenue  = sum(Revenue_DPM);
T = table( ...
    timeGrid_min, price_h, tau_h, ...
    N_frac, N_int, ...
    E_EV_DPM, P_EV_DPM_kW, Revenue_DPM, ...
    'VariableNames',{ ...
    'Time_min','Price','tau_h', ...
    'N_EV_frac','N_EV_int', ...
    'E_EV_DPM_kWh','P_EV_DPM_kW','Revenue_Rs'});

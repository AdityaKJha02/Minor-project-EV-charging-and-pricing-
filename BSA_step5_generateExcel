...
for i = 1:nFCS
    ...
    % Daily energy
BSA.E_FCS_day(i) = sum(BSA.E_FCS{i}, 'omitnan');
...
for i = 1:nFCS
    ...
    % ---- Summary ----
    EV_energy   = dt * sum(BSA.P_EV{i}/eta_EV, 'omitnan');
PV_energy   = dt * sum(BSA.P_PV_avg{i}, 'omitnan');
BC_energy   = dt * sum(BSA.P_BC{i}, 'omitnan');
BD_energy   = dt * sum(BSA.P_BD{i}, 'omitnan');
Grid_energy = BSA.E_FCS_day(i);

summary = {
    'EV_Energy_kWh',          EV_energy;
    'PV_Energy_kWh',          PV_energy;
    'Battery_Charge_kWh',     BC_energy;
    'Battery_Discharge_kWh',  BD_energy;
    'Net_Grid_Energy_kWh',    Grid_energy
};
...
disp('✔ STEP-5 Excel file generated successfully');
end

...
for i = 1:nFCS
    ...
    % --- Extract required columns directly ---
P_EV = M(:,3);     % EV power
P_BC = M(:,5);     % Battery charging power
P_BD = M(:,6);     % Battery discharging power

% Remove NaNs consistently
valid = ~isnan(P_EV);
P_EV = P_EV(valid)/eta_EV;
P_BC = P_BC(valid);
P_BD = P_BD(valid);

% Convert to energy per slot
E_EV_h = P_EV * dt;
E_BC_h = P_BC * dt;
E_BD_h = P_BD * dt;

% --- Grid energy WITHOUT SPV term ---

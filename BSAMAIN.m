...
%% =========================================================
% EXPORT SOC CURVES OF ALL 6 STATIONS TO EXCEL
%% =========================================================

socExcel = 'BSA_SOC_Curves_10min.xlsx';

% Delete old file if it exists
if exist(socExcel,'file')
    delete(socExcel);
end

time_min = (0:10:1430)';    % 144 slots
time_hr  = time_min/60;

for i = 1:nFCS
    
    SOC_curve = BSA.SOC{i}(1:end-1);   % remove extra final point
    ...
    T = table( ...
        ...
        SOC_curve, ...
        'VariableNames', ...
        {'Time_min','Time_hr','SOC_percent'} );
    
    sheetName = sprintf('FCS_%d', i);
    
    writetable(T, socExcel, ...
        'Sheet', sheetName, ...
        'Range','A1');
end

fprintf('✔ SOC curves exported to Excel: %s\n', socExcel);

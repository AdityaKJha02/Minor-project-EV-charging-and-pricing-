function CPP = CPP_from_BSA_Excel(params, BSA_excel)

fprintf('\n===== ENTERING CPP_from_BSA_Excel =====\n');
...
cpp_multiplier = params.cpp_multiplier;
...
while basePrice <= baseMax
    ...
    for i = 1:nFCS
        ...
        %% --------------------------------
        % Generate CPP price profile
        %% --------------------------------
        [~,~,~,~,~,~,price_h] = compute_CPP_tauDriven( ...
    RTP_file, ...
...
CPP.basePrice     = basePrice;
CPP.Total_Revenue = Total_Revenue;
CPP.Total_Cost    = Total_Cost;
CPP.PM            = PM;

...
% FILES
%% =========================================================
EV_file     = 'EVFCS_10min_Arrival_Power_CPP_FINAL.xlsx';
PV_file     = 'PV_Results_6_NPV_10min.xlsx';
RTP_file    = 'RTP_grid_cost_10min.xlsx';
BSA_excel   = 'TEMP_BSA_CPP.xlsx';
...
SOC_init_vec = [50 55 50 55 50 50];
...
%% =========================================================
params.PM_DES       = 0.25;
params.basePrice0   = 8.65; % 8.68 at 1000 as on 07-03
...
params.cpp_multiplier = 2;   % CPP price spike multiplier
...
for k = 1:nB
    ...
    %% --------------------------------
    % RUN CPP OPTIMISATION
    %% --------------------------------
    CPP = CPP_from_BSA_Excel(params, BSA_excel);

    %% --------------------------------
    % STORE RESULTS
    %% --------------------------------
    BasePrice_vec(k) = CPP.basePrice;
    Revenue_vec(k)   = CPP.Total_Revenue;
    TotalCost_vec(k) = CPP.Total_Cost;
    PM_vec(k)        = CPP.PM;

    FixedCost_vec(k) = sum(C_FIX_vec) + sum(C_OandM_vec);
    VarCost_vec(k)   = CPP.Total_Cost - FixedCost_vec(k);
...
outputFile = 'CPP_BESS_Results.xlsx';
...
disp('Excel file generated: CPP_BESS_Results.xlsx');
...
title('CPP Optimized Base Price vs Battery Size');
...
title('CPP Profit Margin vs Battery Size');

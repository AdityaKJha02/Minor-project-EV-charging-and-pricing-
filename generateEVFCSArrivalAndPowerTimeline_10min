...
SOCRangeLabel = strings(nSlots,1);
    ...
    for k = 1:nSlots
        ...
        % -----------------------------------------------
        socRangeStr = strings(portsPerStation(s),1);
...
for e = 1:height(T)
    ...
    if t_now >= t_start && t_now < t_end
        ...
        socRangeStr(p) = socBand + "(EV" + T.evID(e) + ")";
    end
end
    ...
    SOCRangeLabel(k) = strjoin(socRangeStr(usedPorts)', ",");
end
    ...
    ActivePorts, ActiveEVs, SOCRangeLabel, StationPower_kW, ...
    'VariableNames', { ...
    'Hour','Minute','Time_min', ...
    'ActivePorts','EVsCharging','SOC_Range_Charging','StationPower_kW'});

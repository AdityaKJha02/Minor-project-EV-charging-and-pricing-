...
% =========================================================
% SHIFT EXCEL SHEETS TO FIRST NON-ZERO POWER ROW
% =========================================================

filename = 'EVFCS_10min_Arrival_Power.xlsx';

sheetNames_EV = cell(size(chosenEdges,1),1);

% Generate sheet names automatically
for s = 1:size(chosenEdges,1)

    sheetNames_EV{s} = sprintf('EVFCS_%d_%d_%d', ...
...
for s = 1:length(sheetNames_EV)
    ...
    % Column G = StationPower_kW
    colG = 7;

    firstRow = [];

    % -----------------------------------------------------
    % Find first non-zero power row
    % -----------------------------------------------------
    for r = 2:size(data,1)
    ...
    if ~isempty(firstRow)
        ...
        shiftedData = data(firstRow:end,:);

        data = [header; shiftedData];
    ...
    % -----------------------------------------------------
    % Write shifted sheet back
    % -----------------------------------------------------
    writecell(data, filename, 'Sheet', sheet);

end

disp('Shifted EVFCS Excel sheets generated successfully.');
%% =========================================================
%% 12(b) Complete data
%% =========================================================
